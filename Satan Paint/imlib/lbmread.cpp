    ...
    if (memcmp(type,"FORM",4)) {
        ...
        if (memcmp(type,"PBM ",4)) {
            ...
            while (ftell(fp)+4<size) {
                ...
                if (ssize &1)
                    ssize++;            // specs say all chunks are padded by 2
                if (!memcmp(stype,"BMHD",4)) {
                    w = (short) read_other_short(fp);
                    h = (short) read_other_short(fp);
                    x = (short) read_other_short(fp);
                    y = (short) read_other_short(fp);
                    planes=fgetc(fp);
                    masking=fgetc(fp);
                    compr=fgetc(fp);
                    padl=fgetc(fp);
                    tcolor = (short) read_other_short(fp);
                    xa=fgetc(fp);
                    ya=fgetc(fp);
                    pagew = (short) read_other_short(fp);
                    pageh = (short) read_other_short(fp);
                }
                    ...
                    if (im)
                        delete im;  // shouldn't be two BODY's butjust in case...
                    im = new image(w, h);
                    int y;
                    if (!compr) {
                        for (y=0;y<h;h++)
