...
extern char* AddonDir; //js

property_manager *prop;
int *backtiles;
int *foretiles;
JCFont *big_font,*console_font, *game_font;
...
void load_data(int argc, char **argv)
{
    ...
    if (!net_start()) {             // don't let them specify a startup file; we are connecting elsewhere
        if (!main_net_cfg || (main_net_cfg->state != net_configuration::CLIENT && main_net_cfg->state != net_configuration::SERVER))
            for (int i = 1; i < argc; i++) {
                if (!strcmp(argv[i],"-lsf")) {
                    ...
                    if (AddonDir)
                        delete AddonDir;
                    AddonDir = (char*) malloc(strlen(argv[i]) + strlen("addon//") + 1);
                    sprintf(AddonDir, "addon/%s/", argv[i]);
                    sprintf(lsf, "addon/%s/startup.lsp", argv[i]);
                }
            }
    }
    else 
        WriteErr(get_remote_lsf(net_server, lsf), "Unable to get remote lisp startup file from server\n");
    char prog[0xff], *cs;
    ...
    cs = prog;
    if (!eval(compile(cs))) {
        sprintf(prog, "Unable to open lisp startup file '%s'", lsf);
        WriteErr(0, prog);
    }
    compiled_init();
    clear_tmp();
    ...
    WriteErr(start_position_type != 0xffff, "No START object.  Cannot start game.");
    
    sbar.load();
    
    load_number_icons();
    
    WriteErr(nbacktiles, "No background tiles defined!");
    WriteErr(nforetiles, "No foreground tiles defined!");
    WriteErr(foretiles[0] >= 0, "No black (0) foreground tile defined!");
    WriteErr(backtiles[0] >= 0, "No black (0) background tile defined!");
    WriteErr(big_font_pict != -1 || small_font_pict != -1, "No font loaded (use load_big_font or load_small_font)!");
...
char *load_script(char *name)
{
    ...
    WriteErr(s,"Malloc error in load_script");
...
void PreloadData()
{
    ...
    char *cs = "(load \"preload.lsp\")";
    if (!eval(compile(cs))) {
        WriteErr(0, "Error running data preload LISP script\n\nDo you have preload.lsp in the main Abuse directory?");
        return;
    }
