...
void show_end2()
{
    ...
    for (si=sinfo,i=0;i<800;i++) {
        ...
        *(si++)=jrand()%200;     
        *(si++)=c[jrand()%4];     
        screen->putpixel(si[-3], si[-2], (char) si[-1]);
    }
    long paddr[256];
    if (old_pal) {
        for (i=0;i<256;i++) 
    ...
    for (i=0;i<80;) {
        ...
        if (new_time.diff_time(&old_time)>0.1) {
            ...
            for (si=sinfo,j=0;j<800;j++,si+=3)
                screen->putpixel(dx+si[0],dy+si[1], (char) si[2]);
    ...
    int j;
    for (si=sinfo,j=0;j<800;j++,si+=3)
        screen->putpixel(si[0],si[1], (char) si[2]);
    ...
    while (!Done && ev.type != EV_KEY) {
        time_marker now;
        i = (int) (now.diff_time(&start) * 100 / 18);
        screen->clear();
        int j;
        for (si=sinfo,j=0;j<800;j++,si+=3)
            screen->putpixel(dx+si[0],dy+si[1], (char) si[2]);
        ...
        scan_map(screen,ex,ey,cash.img(planet),
            ...
            256,paddr,
            p,cash.img(mask)->height(),eoff,coff);      
	    Done = text_draw(205-i,dx+10,dy,dx+319-10,dy+199,lstring_value(end_plot),wm->font(),cmap,wm->bright_color());
	    wm->flush_screen();
...
void share_end()
{
    ...
    while (!Done && ev.type != EV_KEY) {
        ...
        console_font->put_string(screen,xres/2+35,yres/2+100-console_font->height()-2,
            lstring_value(to_be));
	    Done = text_draw(205-i,dx+10,dy,dx+319-10,dy+199,lstring_value(mid_plot),wm->font(),cmap,wm->bright_color());
	    wm->flush_screen();
