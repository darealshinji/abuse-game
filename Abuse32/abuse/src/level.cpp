    ...
    for (o=first;f && o;o=o->next) {
        ...
        if (f) {
            if (!strcmp(object_names[o->otype],"START")) {
                ...
                f->focus->x=o->x;
                f->focus->y=o->y;          
                f->focus->set_hp((unsigned short) get_ability(f->focus->otype, start_hp));
...
{
    int startable;
    CONDITION(nplayers > 0, "level::make_view_list -- Called with <= 0 players!\n");
    view *f = NULL;
...
game_object *level::boundary_setback(game_object *subject, long x1, long y1, long &x2, long &y2)
{
    ...
    for (;t;t--,blist++) {
        ...
        if (target!=subject && (target->total_objects()==0 || target->get_object(0)!=subject)) {
            target->picture_space(tx1,ty1,tx2,ty2);
            if (!((x2<tx1 && x1<tx1) || (x1>tx2 && x2>tx2) || (y1>ty2 && y2>ty2) || (y1<ty1 && y2<ty1))) { // are they semi/overlapping?
                t_centerx=target->x_center();
                boundary *t_damage;
...
void level::draw_areas(view *v)
{
    ...
    for (;a;a=a->next) {
        ...
        screen->rectangle((short) sx1, (short) sy1, (short) sx2, (short) sy2,c1);
...
level::level(spec_directory *sd, bFILE *fp, char *lev_name)
{
    ...
    if (e) {
        fp->seek(e->offset, 0);
        fg_width = (unsigned short) fp->read_long();
        fg_height = (unsigned short) fp->read_long();
    ...
    if (e) {
        fp->seek(e->offset, 0);
        bg_width = (unsigned short) fp->read_long();
        bg_height = (unsigned short) fp->read_long();
...
int level::load_player_info(bFILE *fp, spec_directory *sd, object_node *save_list)
{
    ...
    if (se) {
        ...
        if (se) {
            fp->seek(se->offset, 0);
            if (fp->read_byte() == RC_L)
                rand_on = (unsigned short) fp->read_long();
        }
        else
            rand_on = 0;
...
void level::write_objects(bFILE *fp, object_node *save_list)
{
    ...
    for (i=0;i<default_simple.total_vars();i++) {
        ...
        for (o=save_list;o;o=o->next) {
            switch (t) {
            case RC_C : {
                fp->write_byte((unsigned char) o->me->get_var(i));
                } break;
            case RC_S : {
                fp->write_short((unsigned short) o->me->get_var(i));
                } break;
            case RC_L : {
                fp->write_long(o->me->get_var(i));
                } break;
            }
        }
    }
...
/* --------------------------------------------------------------------------- 
 ------------------------------------------------------------------------- /**/
void level::hurt_radius(long x, long y,long r, long m, game_object *from, game_object *exclude, int max_push)
{
    if (r < 1)
...
game_object *level::get_random_start(int min_player_dist, view *exclude)
{
    ...
    do {
        ...
        for (; v; v = v->next) {
            if (v != exclude) {
                long cx = abs(v->x_center() - o->x),
                     cy = abs(v->y_center() - o->y),
                     d;
                if (cx < cy)
                    d = cx + cy - (cx >> 1);
                else
...
void level::add_attacker(game_object *who)
{
    if (attack_total >= attack_list_size) { // see if we need to grow the list size..
        attack_list_size++;
        attack_list = (game_object**) jrealloc(attack_list, sizeof(game_object*) * attack_list_size, "attack_list");    
    }
...
void level::add_target(game_object *who)
{
    if (target_total >= target_list_size) { // see if we need to grow the list size..
        target_list_size++;
        target_list = (game_object**) jrealloc(target_list, sizeof(game_object*) * target_list_size, "target_list");    
    }
...
void level::add_block(game_object *who)
{
    if (block_total >= block_list_size) { // see if we need to grow the list size..
        block_list_size++;
        block_list = (game_object**) jrealloc(block_list, sizeof(game_object*) * block_list_size, "block_list");    
    }
...
void level::add_all_block(game_object *who)
{
    if (all_block_total >= all_block_list_size) { // see if we need to grow the list size..
        all_block_list_size++;
        all_block_list = (game_object**) jrealloc(all_block_list, sizeof(game_object*) * all_block_list_size, "all_block_list");    
    }
...
/* --------------------------------------------------------------------------- 
 ------------------------------------------------------------------------- /**/
game_object *level::find_object_in_area(long x, long y, long x1, long y1, long x2, long y2, Cell *list, game_object *exclude)
{
...
/* --------------------------------------------------------------------------- 
 ------------------------------------------------------------------------- /**/
game_object *level::find_object_in_angle(long x, long y, long start_angle, long end_angle, void *list, game_object *exclude)
{
    game_object *closest=NULL;
    long closest_distance=0xfffffff,distance,xo,yo;
    game_object *o=first_active;
