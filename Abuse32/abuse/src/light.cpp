...
{
    next = new light_source((char) type, x, y, inner_radius, outer_radius, xshift, yshift, next);
    return next;
}
...
/* --------------------------------------------------------------------------- 
------------------------------------------------------------------------- /**/
light_source *add_light_source(char type, long x, long y, long inner, long outer, long xshift, long yshift)
{
...
/* --------------------------------------------------------------------------- 
------------------------------------------------------------------------- /**/
void add_light(light_patch *&first, long x1, long y1, long x2, long y2, light_source *who)
{
...
/* --------------------------------------------------------------------------- 
// calculate the light value for this block.  sum up all contributors
------------------------------------------------------------------------- /**/
                            ...
                            long sy)
{
    ...
    for (light_count=lp->total;light_count>0;light_count--)
    {
        ...
        else
        {
            ...
            if (r2<*dt) {                  // if this within the light's outer radius?  (dt==outer_radius)
                int v=*dt-r2; dt++;		
                lv+=v*(*dt)>>16;
            }
        }
  ...
  /* --------------------------------------------------------------------------- 
------------------------------------------------------------------------- /**/
__inline void remap_line_asm2(uchar *addr, uchar *light_lookup, uchar *remap_line, int count)
{
//    int *iaddr = (int*) addr;
    while (count--) {
/*        

//  0.078699243673661588039822623731552 ms/call w/32-bit
        uchar *ioff = (light_lookup + (((long) remap_line[0]) << 8));
        remap_line++;
        int pix32;
        int pixout32;
        pix32 = *iaddr;
        pixout32 = ioff[(uchar) pix32];
        pix32 = pix32 >> 8;
        pixout32 |= (ioff[pix32 & 0xff] & 0xff) << 8;
        pix32 = pix32 >> 8;
        pixout32 |= (ioff[pix32 & 0xff] & 0xff) << 16;
        pix32 = pix32 >> 8;
        pixout32 |= (ioff[pix32 & 0xff] & 0xff) << 24;
        *iaddr = pixout32;
        iaddr++;
        pix32 = *iaddr;
        pixout32 = ioff[(uchar) pix32];
        pix32 = pix32 >> 8;
        pixout32 |= (ioff[(uchar) pix32] & 0xff) << 8;
        pix32 = pix32 >> 8;
        pixout32 |= (ioff[(uchar) pix32] & 0xff) << 16;
        pix32 = pix32 >> 8;
        pixout32 |= (ioff[(uchar) pix32] & 0xff) << 24;
        *iaddr = pixout32;
        iaddr++;
        /**/
        

// 0.15217754014239188063318942664546 ms/call w/out 32-bit
...
void light_screen(image *sc, long screenx, long screeny, uchar *light_lookup, ushort ambient)
{
    ...
    for (int y = cy1; y <= cy2;) {
        ...
        if (suffix) {
            light_patch *lp = f;
            for (;(lp->y1 > y - cy1 || lp->y2 < y - cy1 || lp->x1 > suffix_x || lp->x2 < suffix_x); lp = lp->next);
        ...
        if (prefix) {
            light_patch *lp=f;
            for (;(lp->y1>y-cy1 || lp->y2<y-cy1 || lp->x1>prefix_x || lp->x2<prefix_x);lp=lp->next);
        ...
        for (x = prefix, count = 0; count < remap_size; count++, x += 8, rem++) {
            light_patch *lp = f;
            for (; lp && (lp->y1 > y - cy1 || lp->y2 < y - cy1 || lp->x1 > x || lp->x2 < x); lp = lp->next);
            if (lp)
...
void write_lights(bFILE *fp)
{
    ...
    for (f=first_light_source;f;f=f->next)
    {
        ...
        fp->write_byte((unsigned char) f->type);
    }
}
...
void read_lights(spec_directory *sd, bFILE *fp, char *level_name)
{
    ...
    if (se) {
        ...
        long t = fp->read_long();
        min_light_level = (unsigned short) fp->read_long();
        light_source *last;
        while (t) {
            t--;
            long x = fp->read_long();
            ...
            light_source *p = new light_source((unsigned char) ty, x, y, ir ,ora, xshift, yshift, NULL);
