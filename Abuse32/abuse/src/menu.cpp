...
public:
    VolumeWindow() {
        ...
        inm->add(new ico_button(10, 27, ID_SFX_DOWN,   d_u, d_d, d_ua, d_da,
                 ...
                 NULL)))))));
//        Reconfigure();
        Backg = cash.reg(ff, "vcontrol", SPEC_IMAGE, 1);
        l = cash.img(Backg)->width();
        h = cash.img(Backg)->height();
    ...
    virtual void redraw() {
        cash.img(Backg)->put_image(screen, 0, 0);
        draw_music_vol();
        draw_sfx_vol();
        inm->redraw();
    };
...
VolumeWindow *volume_window = NULL;
...
int menu(void *args, JCFont *font)             // reurns -1 on esc
{
    ...
    int mh = (font->height() + 1) * options + 10,
        maxw = 0;
    ...
    do {
        ...
        if (!last_color_time || (cur_time.diff_time(last_color_time)) > 120) {
            if (last_color_time)
...
{
    draw_vol(6, 16, 34, 22, sfx_volume, 127, pal->find_closest(200, 75, 19),
        pal->find_closest(40, 0, 0));
}
...
{
    draw_vol(6, 61, 34, 67, music_volume, 127, pal->find_closest(255,0,0),
        pal->find_closest(40, 0, 0));
}
...
void show_sell(int abortable)
{
    ...
    if (DEFINEDP(symbol_value(ss))) {
        ...
        while (ss && !quit) {
            ...
            if ((ev.key == JK_ESC || ev.type == EV_MOUSE_BUTTON) && abortable)
                quit = 1;
            fade_out(16);
            ss = CDR(ss);
        }
...
ico_button *make_default_buttons(int x,int &y, ico_button *append_list)
{
    ...
    if (!current_level) {// !main_net_cfg || (main_net_cfg->state != net_configuration::SERVER && main_net_cfg->state != net_configuration::CLIENT)) {
        set = new ico_switch_button(x, y, ID_NULL, diff_on,
                                    ...
                                    load_icon(10, ID_EXTREME, x, y, h, NULL,"ic_extreme"), "ic_hard"), "ic_medium"), "ic_easy"), NULL);
        y += h;
    }
    ...
    ico_button *Networking = NULL;
    if (ProtocolExists()) {
        Networking = load_icon(11, ID_NETWORKING, x, y, h, NULL, "ic_networking");
        y += h;
    }
    ...
    color->next = volume;
    if (Networking) {
        volume->next = Networking;
        Networking->next = sell;
    }
    else
        volume->next = sell;
    sell->next = quit;
