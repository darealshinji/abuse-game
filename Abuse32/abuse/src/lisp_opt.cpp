...
void *comp_optimize(void *list)
{
    ...
    if (list) {
        if (CAR(list)==if_symbol) {
            ...
            else if (lcar(list)==eq_symbol && (lcar(lcdr(lcdr(list)))==zero_symbol)) { //simplify (eq x 0)-> (eq0 x)
            ...
            else if (lcar(eval1)==progn_symbol && (eval2==NULL || item_type(eval2)!=L_CONS_CELL)) {
