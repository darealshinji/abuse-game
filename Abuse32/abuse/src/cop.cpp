    ...
    if (o->total_objects()) {           // make sure we are linked to the main character
        ...
        if (v) {
            if (!v->freeze_time) {
                ...
                rand_on += (short) o->lvars[point_angle];
...
void *plaser_ufun(void *args)
{
    ...
    if (signal==l_FIRE) {
        if (!o->lvars[fire_delay1]) {                  // make sur we are not waiting of previous fire
            ...
            ret = new_lisp_number(0);                // can't fire yet, return 0 ammo subtract
    }
    return ret;
}
                ...
                if (bad_guy_array) {
                    ...
                    for (p=current_level->first_active_object();p;p=p->next_active) {
                        ...
                        if (xd<160 && yd<130 && bad_guy_array[p->otype] && p!=other) {
                            if (p->targetable() && !(p->otype==S_ROCKET && p->total_objects() && p->get_object(0)==bot)) { // don't track onto own missles
                                d = xd * xd + yd * yd;
                                if (d<cl) {
                                    cl = d;
                                    target = p;
...
void *player_draw(int just_fired_var, int num)
{
  ...
  /*
Leon's suggestion for player 0 tinting 


///// -----------------------------------------
fix for no player 0 tint
///// -----------------------------------------  
I can imagine the problem. The color for the 1st player is fluorescent and random right? I people.lsp to the top there is a list of tints. The first in the list is "0". Change this to 
(def_tint "art/ball.spe") 
and the color -should- be back to normal. 

This is what the array will look like: 

(setq player_tints (make-array 8 :initial-contents (list 
(def_tint "art/ball.spe") ; 0 default 
...
void *top_draw()
{
    ...
    if (o->total_objects()) {
        ...
        if (bot->state == stopped  || bot->state == running || bot->state == run_jump || bot->state == run_jump_fall || bot->state==end_run_jump) {
            int oldy = o->y;
            o->x = bot->x;
            if (bot->direction < 0)
                o->x += 4;
            ...
            push_onto_list(new_lisp_number(bot->GetTint()), ret);
            
            if (bot->lvars[special_power] == SNEAKY_POWER) {
                if (bot->lvars[used_special_power] == 0)
                    player_draw(top_just_fired, bot->GetTint());
...
//    game_object *bot = o->get_object(0);
    ...
    if (o->aistate() > 0) {
        switch (o->lvars[special_power]) {
            case NO_POWER:
                player_draw(just_fired, o->GetTint());
                break;
            
            case HEALTH_POWER: {
                    player_draw(just_fired, o->GetTint());
                    if (o->controller() && o->controller()->local_player())
                        cash.img(S_health_image)->put_image(screen, o->controller()->cx2 - 20, o->controller()->cy1 + 5, 1);
                } break;
            case FAST_POWER : {
                    ...
                    if (o->controller() && o->controller()->local_player())
                        cash.img(S_fast_image)->put_image(screen, o->controller()->cx2 - 20, o->controller()->cy1 + 5, 1);
                } break;
            case FLY_POWER : {
                    int old_state = o->state;
                    switch (o->state) {
                    ...
                    if (o->controller() && o->controller()->local_player())
                        cash.img(S_fly_image)->put_image(screen,o->controller()->cx2-20, o->controller()->cy1 + 5, 1);
                } break;
            case SNEAKY_POWER : {
                    if (o->lvars[used_special_power]==0)
                        player_draw(just_fired, o->GetTint());
                    ...
                    if (o->controller() && o->controller()->local_player())
                        cash.img(S_sneaky_image)->put_image(screen, o->controller()->cx2 - 20, o->controller()->cy1 + 5, 1);
                } break;
            }    
    }
    return NULL;
...
void *score_draw()
{
    ...
    if (local) {
        ...
        int i;
        for (i = 0; i < tp; i++) {
            int color=lnumber_value(lget_array_element(symbol_value(l_player_text_color), sorted_players[i]->GetTint()));
...
void *show_kills()
{
    ...
    for (i=0;i<tp;i++) {
        enum { NAME_LEN = 18 } ;
        int color = lnumber_value(lget_array_element(symbol_value(l_player_text_color),v->GetTint()));  
