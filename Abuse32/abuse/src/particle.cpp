...
int defun_pseq(void *args)
{
    ...
    if (item_type(sym)!=L_SYMBOL) {
        ...
        WriteErr(0, "defun_pseq -- Expecting first arg to def-particle to be a symbol!");
    }
...
part_sequence::part_sequence(void *args)
{
    ...
    if (fp->open_failure()) {
        delete fp;
        lprint(args);
        sprintf(ErrMsg, "\nparticle sequence : Unable to open file %s for reading\n",fn);
        WriteErr(0, NULL);        
    }
...
void part_frame::draw(image *screen, int x, int y, int dir)
{
    ...
    if (x + x1 > cx2 || x + x2 < cx1 || y + y1 > cy2 || y + y2 < cy1)
        return;
