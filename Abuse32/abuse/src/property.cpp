        ...
        while (!feof(fp)) {
            if (fgets(buf,100,fp)) {
                for (c1=buf,c2=name;*c1 && *c1!='=';c1++,c2++)
                    *c2=*c1;
                if (*c1 == 0) {
                    sprintf(ErrMsg, "Missing = for property line %s in file %s\n",buf,filename); 
                    WriteErr(0, NULL);
                }
                *c2=' ';
                while (*c2 == ' ') {
                ...
                if (*c1=='"') {
                    ...
                    *c2=0;
                    if (*c1!='"') {
                        sprintf(ErrMsg,"Missing \" for property name %s in file %s\n",name,filename);
                        WriteErr(0, NULL);
                    }
                    set(name,str);
                }
                    ...
                    if (sscanf(c1,"%lg",&x))
                        ...
                        sprintf(ErrMsg, "Bad number/string for property name %s in file %s\n",name,filename);
                        WriteErr(0, NULL);
                    }	  
                }			  
            }
