...
boundary::boundary(bFILE *fp, char *er_name) : point_list(fp)
{
    ...
    if (tot) {
        if (!(data[0] == data[(tot - 1) * 2] && data[1] == data[tot * 2 - 1])) {
            sprintf(ErrMsg, "%s : Endpoints of foretile do not match start points\n",er_name);
            WriteErr(0, NULL);
        }
...
foretile::foretile(bFILE *fp)
{
    ...
    if (!pal) {
        lbreak("Palette has not been defined\nUse load_palette before load_tiles");
        //js exit(0);
    }

    if (!color_table) {
        lbreak("Color filter has not been defined\nUse load_color_filter before load_tiles");
        //js exit(0);
    }
...
int figure::size()
{
    return forward->size() + backward->size() + hit->size() + f_damage->size() + b_damage->size() + sizeof(figure);
}
...
char_tint::char_tint(bFILE *fp)  // se should be a palette entry
{
    ...
    for (int i = 0; i < 256; i++, t++, p_addr += 3)  
        *t = pal->find_closest(p_addr[0], p_addr[1], p_addr[2]);
    delete p;  
}
