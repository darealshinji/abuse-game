...
int nfs_file::unbuffered_read(void *buf, size_t count)      // returns number of bytes read
{
    if (local)
        ...
        if (nfs_fd >= 0) {
            size_t a = NF_read(nfs_fd, buf, count);
            if (a > count) {
                dprintf("nfs_file::unbuffered_read -- Read too much from file; buffer overrun\n");
            }
            return a;
        }
        else 
...
int nfs_file::unbuffered_write(void *buf, size_t count)      // returns number of bytes written
{
    if (local)
        ...
        dprintf("nfs_file::unbuffered_write -- Write to nfs file not allowed for now!\n");
    }
    return 0;
}
