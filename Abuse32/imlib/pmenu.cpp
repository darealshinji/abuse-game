...
{ 
    ...
    if (cx1<X)
        cx1 = X;
    int w = cx2 - cx1 - jwindow::LeftBorder() - jwindow::RightBorder() + 1;
    int h = jwindow::TopBorder() + jwindow::BottomBorder();

    bar = wm->new_window(X, Y, w, 0, NULL);
    bar->set_moveability(0);  // can't drag this window
    bar->screen->wiget_bar(0, 0, w - 1, h - 1, wm->bright_color(), wm->medium_color(), wm->dark_color());
...
{
    if (key==hotkey && hotkey!=-1)
        return this;
    else if (sub)
        return sub->find_key(key);
    else
        return NULL;
}
...
void psub_menu::draw(jwindow *parent, int x, int y)
{
    ...
    if (h+y+parent->y>cy2)
        if (parent->y+parent->h+wm->font()->height()>cy2)
            ...
            y=y-h+wm->font()->height()+5;

    win = wm->new_window(parent->x + x, parent->y + y,
        w - jwindow::LeftBorder() - jwindow::RightBorder(), h - jwindow::TopBorder() - jwindow::BottomBorder(), NULL);
...
void pmenu_item::draw_self(jwindow *parent, int x, int y, int w, int top, int active)
{
    ...
    if (!n) {
        ...
        if (active) {
            if (xp != -1)
                parent->screen->xor_bar(bx, y, x + w - 1, y + wm->font()->height(), wm->dark_color());
            else {
                parent->screen->bar(bx, y, x + w - 1, y + wm->font()->height(), wm->dark_color());
                wm->font()->put_string(parent->screen, x + 1, y + 1, n, wm->medium_color());	
                if (on_off && *on_off)
                    wm->font()->put_string(parent->screen, bx + 1, y + 1, "*", wm->medium_color());
            }
        }
        else {
            if (xp != -1)
                ...
                parent->screen->bar(bx, y, x + w - 1, y + wm->font()->height(), wm->medium_color());
                wm->font()->put_string(parent->screen, x + 1, y + 1, n, wm->bright_color());
                if (on_off && *on_off)
                    wm->font()->put_string(parent->screen, bx + 1, y + 1, "*", wm->bright_color());
            }
        }
    }
}
...
/* --------------------------------------------------------------------------- 
 ------------------------------------------------------------------------- /**/
void pmenu_item::draw(jwindow *parent, int x, int y, int w, int top, int active)
{  
    if (n) {
        if (active) {
...
int pmenu::itemx(pmenu_item *p)
{
    ...
    int total = 0, tw, i = 0, x;
    pmenu_item *pp;
    for (pp = top; pp; pp = pp->next, i++) {
        if (pp == p)
            x = i;
        total++;
    }
...
/* --------------------------------------------------------------------------- 
 ------------------------------------------------------------------------- /**/
int pmenu_item::handle_event(jwindow *parent, int x, int y, int w, int top, event &ev)
{
    x+=parent->x;
    y+=parent->y;
