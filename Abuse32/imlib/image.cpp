...
/* --------------------------------------------------------------------------- 
short current_error()
    Returns the code of the last error that happened
 ------------------------------------------------------------------------- /**/
short current_error()
{ 
    return imerror;
...
/* --------------------------------------------------------------------------- 
void clear_errors()
    Aborts program, outputting error code
 ------------------------------------------------------------------------- /**/
void clear_errors()
{
    if (imerror) {
        ...
        if (imerror <= imMAX_ERROR)
            sprintf(msg, "Program stopped.  Error: %s\n", imerr_messages[imerror]);
        else
            strcpy(msg, "Unsponsored error code, you've got trouble\n");
        WriteErr(0, msg);
    }
}
...
/* --------------------------------------------------------------------------- 
short last_error()
    Returns last error code, then clears it
 ------------------------------------------------------------------------- /**/
short last_error()
{
    short ec;
...
/* --------------------------------------------------------------------------- 
 ------------------------------------------------------------------------- /**/
image_descriptor::image_descriptor(short length, short height, int keep_dirties, int static_memory)
{ 
    clipx1 = 0; 
    clipy1 = 0; 
    l = length; 
...
{
    CONDITION(x >= 0 && x < width() && y >= 0 && y < height(), "image::pixel Bad pixel xy");
...
{
    CONDITION(x>=0 && x<width() && y>=0 && y<height(), "image::putpixel Bad pixel xy");
    if (special) {
        if (x >= special->x1_clip() && x <= special->x2_clip() && y >= special->y1_clip() && y <= special->y2_clip())
            (*(scan_line(y) + x)) = color;
    }
    else 
        (*(scan_line(y) + x)) = color;
...
image::image(short width, short height, unsigned char *page_buffer, short create_descriptor)
{
    ...
    CreateDDSurface(create_descriptor);
    if (create_descriptor) {    
        HRESULT hRet;
        HRGN hRgn;
        BYTE RgnData[1024];
        RGNDATA *Region = (RGNDATA*) RgnData;
        LPDIRECTDRAWCLIPPER SurfClip = NULL;

        hRgn = CreateRectRgn(0, 0, w, h);
        GetRegionData(hRgn, sizeof(RgnData), Region);
        DeleteObject(hRgn);

        hRet = g_pDD->CreateClipper(0, &SurfClip, NULL);
        hRet = SurfClip->SetClipList(Region, 0);
        hRet = Surf->SetClipper(SurfClip);
        SurfClip->Release();
    }
}
...
void image_init()
{
    ...
    wrd = int_to_intel(*up);
    if (wrd != 0x01) {
        WriteErr(0, "Major error.  Compiled under wrong ENDIAN-nes.");
        printf("Compiled under wrong ENDIAN-nes, edit system.h and try again\n");
        printf("1 (intel) = %d\n", (int) wrd);
    }
    imerror = 0;
}
...
void image::line(short x1, short y1,short x2, short y2, unsigned char color)
{
    ...
    // see if it got cliped into the box, or out
...
void image::put_image(image *screen, short x, short y, char transparent)
{
    ...
    if (screen->special)  // the screen is clipped then we onl want to put
        ...
        if (screen->Surf && Surf) {
            HRESULT hRet;
            RECT drect, srect;
            drect.left = x; drect.right = x + w; drect.top = y; drect.bottom = y + h;
            srect.left = 0; srect.right = w; srect.top = 0; srect.bottom = h;

            hRet = screen->Surf->Blt(&drect, Surf, &srect, 0, NULL);
            if (hRet == DD_OK)
                return;
            switch (hRet) {
        ...
        if (x<screen->width() && y<screen->height()) {
            ...
            for (j = starty; j < yl; j++, y++) {
	            ...
	            else memcpy(&pg1[x], pg2, xl);   // straight copy
            }
            UnLock();
            screen->UnLock();
        }
		...
		short x1, short y1, short x2, short y2, char transparent)
{
    ...
    if (y2 >= height()) 
        y2 = height() - 1;
    if (x1 > x2 || y1 > y2) 
        return;      // return if it was adjusted so that nothing will be put
    ...
    if (screen->Surf && Surf) {
        HRESULT hRet;
        RECT drect, srect;
        drect.left = x; drect.right = x + xlen; drect.top = y; drect.bottom = y + ylen;
        srect.left = x1; srect.right = x1 + xlen; srect.top = y1; srect.bottom = y1 + ylen;
        if (transparent) {
            DDBLTFX bltfx;
            ZeroMemory(&bltfx, sizeof(bltfx));
            bltfx.dwSize = sizeof(bltfx);
            bltfx.ddckSrcColorkey.dwColorSpaceHighValue = current_background;
            bltfx.ddckSrcColorkey.dwColorSpaceLowValue = current_background;
            hRet = screen->Surf->Blt(&drect, Surf, &srect, DDBLT_KEYSRCOVERRIDE, &bltfx);
        }
        else
            hRet = screen->Surf->Blt(&drect, Surf, &srect, 0, NULL);
        if (hRet == DD_OK)
		...
		short x1, short y1, short x2, short y2, char transparent)
{
    ...
    i = x1; 
    x1 = width() - x2 - 1;  // reverse the x locations
    x2 = width() - i - 1;
    ...
    if (x < screen->width() && y < screen->height() && x1 < width() && y1 < height() && x1 <= x2 && y1 <= y2) {
        if (x2>=width())
            x2=width()-1;
        if (y2>=height())
            y2=height()-1;
...
{ 
    ...
    short i, j;
    long brv, x;
    brv = (long) 258 * (long) 258 * (long) 258; 
    bri = 0;
    Lock();
    for (j=0;j<h;j++) {
        p = scan_line(j);
...
void image::set_clip(short x1, short y1, short x2, short y2)
{
    ...
    if (!special) {
    // create a new image descriptor with no dirty rectangle keeping
...
void image::scroll(short x1, short y1, short x2, short y2, short xd, short yd)
{
    ...
    short xsrc, ysrc, xdst, ydst, xtot, ytot;
    xtot = x2 - x1 - abs(xd) + 1;
    if (xd < 0) {
        xsrc = x1 - xd;
        xdst = x1; 
    ...
    ytot = y2 - y1 - abs(yd);
    this->put_part(this, xdst, ydst, xsrc, ysrc, xsrc + xtot - 1, ysrc + ytot - 1, 0);
}
...
/* --------------------------------------------------------------------------- 
 ------------------------------------------------------------------------- /**/
void image::wiget_bar(short x1, short y1, short x2, short y2, unsigned char light, unsigned char med, unsigned char dark)
{
    line(x1, y1, x2, y1, light);
    line(x1, y1, x1, y2 - 1, light);
    line(x2, y1 + 1, x2, y2, dark);
    line(x1, y2, x2 - 1, y2, dark);
...
void image::burn_led(short x, short y, long num, short color, short scale)
{
    ...
    for (xx=0;xx<8;xx++) {
        if (st[xx]!=' ') {
            ...
            for (yy=0;yy<7;yy++)
	            if ((1<<yy)&dig[zz])
	                line(x+ledx[yy*2]*scale,y+ledy[yy*2]*scale,x+ledx[yy*2+1]*scale, y+ledy[yy*2+1]*scale,(unsigned char) color);
        }
        x += 6 * scale;
    }
}
...
/* --------------------------------------------------------------------------- 
 ------------------------------------------------------------------------- /**/
void image::CreateDDSurface(int mode)
{
    if (Surf)
        Surf->Release();
    Surf = NULL;
...
//    ddsd.ddsCaps.dwCaps |= DDSCAPS_SYSTEMMEMORY;
    ...
    hRet = g_pDD->CreateSurface(&ddsd, &Surf, NULL);
    hRet = Surf->Lock(NULL, &ddsd, 0, NULL);

    unsigned char *line_addr;
    line_addr = (unsigned char*) ddsd.lpSurface;
    if ((DWORD) ddsd.lPitch == ddsd.dwWidth)
        memcpy(line_addr, data, (w * h));
    else {
        unsigned char* im_addr = data;
        int i;
        for (i = 0; i < h; i++) {
            memcpy(line_addr, im_addr, w);
            im_addr += w;
