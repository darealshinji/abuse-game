...
/* 
  ...
  6/18/2000 - Jeremy Scott
    Began tidying up code

  6/17/2000 - Jeremy Scott
    morph.cpp appears to be the place to start looking for the rendering code

  11/24/2000 - Jeremy Scott
    This isn't the rendering code.
  */
...
/* --------------------------------------------------------------------------- 
void jmorph::show_step_frame
    6/18/2000 - JS
      Appears to be an alpha-blend fading function 
 ------------------------------------------------------------------------- /**/
    ...
    if (small) {
        ...
        for (i = 0; i < total; i++, m8++) {
            ...
            if (xx >= x1 && xx <= x2 && yy >= y1 && yy <= y2) {
                ...
                *(screen->scan_line((short) yy) + xx) = fli->lookup_color(r, g, b);
            }
        }
        screen->UnLock();
    }
        ...
        for (pixely = 0; pixely < h; pixely++) {
            for (pixelx = 0; pixelx < w; pixelx++) {
                if (middle_map[pixelx + pixely * w] == NULL) { // we are checking for 'duds'  (misplaced black pixels)
	                int tot;	
	                if (pixelx>0) 
                        if (middle_map[pixelx-1+pixely*w]) 
                            tot=1; 
                        else 
                            tot=0;      
	                if (pixelx<w-1) 
                        if (middle_map[pixelx+1+pixely*w]) 
                            tot++;
	                if (pixely>0) 
                        if (middle_map[pixelx+(pixely-1)*w]) 
                            tot++;	
	                if (pixely<h-1) 
                        if (middle_map[pixelx+(pixely+1)*w]) 
                            tot++;	
               ...
               int aneal_steps)
{
    ...
    for (x=0;x<256;x++) 
    {
        if ((hint_hist1[x]!=0 && hint_hist2[x]==0) || (hint_hist1[x]==0 && hint_hist2[x]!=0)) {
            sprintf(ErrMsg, "Color %d used for hinting is present in one hint mask, but not the other\n",x);
            WriteErr(0, NULL);
        }
    ...
    if (mor->small_morph()) {
        ...
        if (frame_direction>0) {
            ...
            for (i=0;i<total;i++,m8++) {
	            ...
	            points[i].dr= ((int)r2-(int)r1)<<5;
	            points[i].dg= ((int)g2-(int)g1)<<5;
	            points[i].db= ((int)b2-(int)b1)<<5;			
            }
        }
        else {
	        morph_point8 *m8 = mor->small_points();      
