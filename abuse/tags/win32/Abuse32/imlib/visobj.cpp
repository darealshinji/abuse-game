...
/* --------------------------------------------------------------------------- 
 ------------------------------------------------------------------------- /**/
void image_visual::draw(image *screen, int x, int y, filter *f)
{ 
    if (f)
...
/* --------------------------------------------------------------------------- 
 ------------------------------------------------------------------------- /**/
static void put_para(image *screen, char *st, int dx, int dy, int xspace, int yspace, JCFont *font, int color)
{
    int ox=dx;
    while (*st) {
        if (*st=='\n') {
...
{ 
    put_para(screen,st,x+1,y+1,wm->font()->width(),wm->font()->height(), wm->font(),f ? f->get_mapping(color) : color);
}