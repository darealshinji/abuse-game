...
{
/*js  Killing to finish off last annoying build warning */
#ifndef _WIN32
  memory_node *f=sfirst;
...
void block_manager::init(void *block, long Block_size, uchar type)
{
  ...
  /* 
     ...
     in jfree to determine whether a pointer is too small an object or a large alloc
     and it must always be true that the address of the pointer is > JM_SMALL_SIZE 
     All systems I know start pointer address pretty high, but this is a porting concern.     
  */
  
  slast = sfirst = (memory_node *)(((char *)block)+JM_SMALL_SIZE);   
  sfirst->size = -(block_size- (signed) sizeof(memory_node)-JM_SMALL_SIZE);
  sfirst->next = NULL;
  cfirst = NULL;
...
void *block_manager::static_alloc(long size, char *name)
{
  ...
  {    
    ...
    if (s==slast)
      slast=p;
    p->size=-(s->size-size-(signed) sizeof(memory_node));
#ifdef MEM_CLEAR
    ...
    for (int i=0;i<bmanage_total;i++)
        if (ptr>=(void *)bmanage[i].sfirst) { // is the pointer in this block?
            if (ptr<=(void *)bmanage[i].slast) { // is it in static space?
                ...
                if (ptr<=(void *)(((char *)bmanage[i].sfirst)+bmanage[i].block_size)) { // or cache space?
