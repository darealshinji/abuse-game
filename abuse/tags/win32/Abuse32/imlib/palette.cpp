    ...
    if (f<0) {
        if (u>=0) {
            ...
            for (i=0,f=0,u=10000;i<ncolors;i++) {
                c=(pal[i].red-r)*(pal[i].red-r) + (pal[i].green-g)*(pal[i].green-g) + (pal[i].blue-b)*(pal[i].blue-b);
                if (c<u) {
                    f=i;
                    u=c;
                }
...
palette *quant_palette::create_pal()
{
    ...
    p = new palette(mx);
    for (x=0,i=7;i>=0;i++)
        for (pn = (quant_node*) level[i].first(); pn != (quant_node*) level[i].first(); pn = (quant_node*) pn->next())
            if (pn->is_leaf())
