...
int view::handle_event(event &ev)
{
    ...
    if (ev.type == EV_KEY) {
        ...
        if (total_weapons && ev.mouse_move.z)
            if (ev.mouse_move.z > 0) {
                while (ev.mouse_move.z) {
                    next_weapon();
                    ev.mouse_move.z--;
                }
                return 1;
...
void set_local_players(int total)
{
    ...
    for (view *f=player_list;f;f=f->next) {
        if (total && f->local_player())
            ...
            if (last)
                last->next = n;
            else if (n)    // make sure we have at least one local player
                player_list = n;
            last = f;
            rdw = 1;
        }
    ...
    while (total) {  // see if we need to add new players
        ...
        v->cy1 = 200 / 2 - 95;
        v->cx2 = 320 / 2 + 155;
        v->cy2 = 200 / 2 + (total_weapons ? 60 : 95);
...
long view::set_view_var_value(int num, long x)
{
    switch (num) {
        ...
        case V_AMBIENT : ambient = (short) x; break;
...
void view::configure_for_area(area_controller *a)
{
    if (a->ambient>=0 && a->ambient!=ambient) {
        if (ambient>a->ambient) {
            ambient -= (short) a->ambient_speed;
            if (ambient<a->ambient) 
                ambient = (short) a->ambient;
        }
        else {
            ambient += (short) a->ambient_speed;
            if (ambient>a->ambient) 
...
/* --------------------------------------------------------------------------- 
 ------------------------------------------------------------------------- /**/
void view::SetTint(int NewTint)
{
    if (NewTint < 0)
        NewTint = 0;
    Tint = NewTint;
    focus->SetTint(NewTint);
}
/* --------------------------------------------------------------------------- 
 ------------------------------------------------------------------------- /**/
int view::GetTint()
...
/* --------------------------------------------------------------------------- 
 ------------------------------------------------------------------------- /**/
void view::SetTeam(int NewTeam)
{
    Team = NewTeam;
    focus->SetTeam(NewTeam);
}
/* --------------------------------------------------------------------------- 
 ------------------------------------------------------------------------- /**/
int view::GetTeam()
