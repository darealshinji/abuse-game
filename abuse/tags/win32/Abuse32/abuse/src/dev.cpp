...
char level_file[1024] = "levels/level00.spe";
...
int confirm_quit()
{
    ...
    jwindow *quitw = wm->new_window(xres / 2 + 40, yres / 2, -3, -3,
...
void scale_put_trans(image *im, image *screen, int x, int y, short new_width, short new_height)
{
    ...
    short cx1, cy1, cx2, cy2;
    screen->get_clip(cx1, cy1, cx2, cy2);
    if (cx1 > cx2 || cy1 > cy2 || x > cx2 || y > cy2 || x + new_width <= cx1 || y + new_height <= cy1)
        return;
    if (x < cx1) {
        ix_start = (cx1- x) * xstep;
        new_width -= (cx1 - x);
...
void dev_controll::dev_draw(view *v)
{
    ...
    if (dev&EDIT_MODE) {
        ...
        if (dev&DRAW_LINKS) {
            for (light_source *f=first_light_source;f;f=f->next) {
                if (f->x-vx>=0 && f->x-vx<=(v->cx2-v->cx1+1) && f->y-vy>=0 && f->y-vy<=(v->cy2-v->cy1+1)) {
                    ...
                    screen->rectangle((short) (f->x1 - vx + v->cx1), (short) (f->y1 - vy + v->cy1), (short) (f->x2 - vx + v->cx1), (short) (f->y2 - vy + v->cy1), wm->medium_color());
                }
            }
        }
        ...
        if (link_object) {
            ...
            screen->line((short) rx1, (short) ry1, dlastx, dlasty, yellow);            
        }
        ...
        if (show_names)
            ...
            if (dev&DRAW_LINKS) {
                ...
                for (o=current_level->first_object();o;o=o->next) {
                    ...
                    for (;i<o->total_objects();i++) {
                        ...
                        screen->line((short) x1, (short) y1, (short) x2, (short) y2, wm->bright_color());
                    }
                    
                    for (i=0;i<o->total_lights();i++) {
                        light_source *l=o->get_light(i);
                        the_game->game_to_mouse(l->x,l->y,current_view,x2,y2);
                        screen->line((short) x1, (short) y1, (short) x2, (short) y2, light_connection_color);
                    }
                }
            }
            ...
            if (selected_object) {
                ...
                screen->rectangle((short) rx1, (short) ry1, (short) rx2, (short) ry2, wm->bright_color());
...
void dev_controll::toggle_toolbar()
{
    if (tbw) {
        ...
        tool_picker *tp=new tool_picker(0, 0, ID_NULL, 5, (visual_object**) dev_mode_pict, dev_mode_ids, DEV_MODES, pal, pal, NULL);
        tbw = wm->new_window(prop->getd("toolbar x", -1), prop->getd("toolbar y",-1), -1, -1, tp);
...
void dev_controll::toggle_show_menu()
{
    if (show_menu) {
        ...
        button *lnb = new button(0, 100, SHOW_LINKS,            symbol_str("l_links"),NULL); if (dev & DRAW_LINKS)          lnb->push();
        button *lb  = new button(0, 80,  SHOW_LIGHT,            symbol_str("l_light"),lnb);  if (dev & DRAW_LIGHTS)         lb->push();
        button *cb  = new button(0, 60,  SHOW_CHARACTERS,       symbol_str("l_char"),lb);    if (dev & DRAW_PEOPLE_LAYER)   cb->push();
        button *bb  = new button(0, 40,  SHOW_BACKGROUND,       symbol_str("l_back"),cb);    if (dev & DRAW_BG_LAYER)       bb->push();
        button *bdb = new button(0, 20,  SHOW_FOREGROUND_BOUND, symbol_str("l_bound"),bb);   if (dev & DRAW_FG_BOUND_LAYER) bdb->push();
        button *fb  = new button(0, 0,   SHOW_FOREGROUND,       symbol_str("l_fore"),bdb);   if (dev & DRAW_FG_LAYER)       fb->push();
        
        show_menu=wm->new_window(prop->getd("layer x",-1), prop->getd("layer y",-1), -1, -1, fb, symbol_str(symbol_str("SHOW?")));
    }
}
    ...
    if (omenu) {
        ...
        for (i = 0; i < total_objects; i++)
            ...
            omenu = wm->new_window(prop->getd("objects x",0),
                ...
                new pick_list(0, 0, DEV_CREATE, yres / wm->font()->height() / 2,
...
void dev_controll::toggle_pmenu()
{
    if (pmenu) {
        ...
        pmenu = wm->new_window(prop->getd("pal x", 0), prop->getd("pal y", -1), -1, -1,
                                new pick_list(0, 0, DEV_PALETTE, yres / wm->font()->height() / 2,
...
void dev_controll::toggle_bgw()
{
    if (!backw) {
        ...
        backw = wm->new_window(prop->getd("back x", -30), prop->getd("back y",0), -1, -1,
                new tile_picker(0, 0, DEV_BG_PICKER, SPEC_BACKTILE, bg_scale, maxh, bg_w, NULL), symbol_str("l_bg"));
    }
    else {
        backw_on = 1;
...
void dev_controll::toggle_search_window()
{
    if (!search_window) {
        ...
        search_window = wm->new_window(prop->getd("searchw x",-30), prop->getd("searchw y",0),-1,-1,
            new text_field(0, 0, ID_SEARCH_TEXT,"object name>", "***************************", prop->get("search name",""),
            new button(bw,     wm->font()->height() + 5, ID_SEARCH_BACKWARD, cash.img(dev_backward),
            new button(bw * 3, wm->font()->height() + 5, ID_SEARCH_FOREWARD, cash.img(dev_forward), NULL))),"SEARCH");
        searchw_on = 0;
    }
    else {
        searchw_on = 1; 
...
int open_owin = 0, open_fwin = 0, open_bwin=0, start_edit = 0, start_nodelay = 0, 
start_doubled = 0, start_mem = 0;
...
void dev_controll::do_command(char *command, event &ev)
{
    ...
    if (!strcmp(fword,"fg_select")) {
        ...
        if (x>=0 && y>=0 && x<current_level->foreground_width() && y<current_level->foreground_height()) {
...
void dev_controll::toggle_light_window()
{
    if (!lightw) {
        ...
        lightw = wm->new_window(prop->getd("light create x",0),
               ...
               new text_field(0, bh * 4 + th *2, DEV_LIGHTR1, "R1","******", prop->getd("light create r1",1),
               new text_field(0, bh * 4 + th *3, DEV_LIGHTR2, "R2","******", prop->getd("light create r2",100),
               NULL))))), symbol_str("l_light"));
    }
    else {
...
void dev_controll::make_ai_window(game_object *o)
{
    ...
    if (figures[o->otype]->total_fields) {
        ...
        aiw = wm->new_window(prop->getd("ai x", 0), prop->getd("ai y", 0), -1, -1,
    ...
    if (aiw) {
        ...
        if (o) {
            if (figures[o->otype]->total_fields) {
                ...
                x = atoi(aiw->read(DEV_AI_XVEL)); if (x!=o->xvel()) o->set_xvel((unsigned short) x);
                x = atoi(aiw->read(DEV_AI_YVEL)); if (x!=o->yvel()) o->set_yvel((unsigned short) x);
                
                x = atoi(aiw->read(DEV_AI_XACEL)); if (x!=o->xacel()) o->set_xacel(x);
                x = atoi(aiw->read(DEV_AI_YACEL)); if (x!=o->yacel()) o->set_yacel(x);
                
                x = atoi(aiw->read(DEV_AI_STTIME)); if (x!=o->aistate_time()) o->set_aistate_time((unsigned short) x);
                x = atoi(aiw->read(DEV_AI_GRAVITY)); if (x!=o->gravity()) o->set_gravity(x);
                
                x = atoi(aiw->read(DEV_AI_HEALTH)); if (x!=o->hp()) o->set_hp((unsigned short) x);
                x = atoi(aiw->read(DEV_AI_MORPHPR)); if (x!=o->mp()) o->set_mp((unsigned short) x);
                
                x = atoi(aiw->read(DEV_AI_TYPE)); if (x!=o->aitype()) o->set_aitype((unsigned char) x);
                x = atoi(aiw->read(DEV_AI_STATE)); if (x!=o->aistate()) o->set_aistate((unsigned short) x);
                x = atoi(aiw->read(DEV_AI_FADE)); if (x!=o->fade_count()) o->set_fade_count((unsigned char) x);
            }
        }
...
void dev_controll::pick_handle_input(event &ev)
{
    ...
    if (ev.type==EV_MOUSE_BUTTON && ev.mouse_button) {
        ...
        if (find && current_area && dc) {
            ...
            int wl = 0;
            int wh = 0, th = wm->font()->height() + 12, bw = cash.img(dev_ok)->width() + 10;
            area_win = wm->new_window(prop->getd("area_box x", 0),
                prop->getd("area_box y", 0), -1,-1,
...
void dev_controll::handle_event(event &ev)
{
    ...
    if (dev_menu && dev_menu->handle_event(ev, screen))
        return;
            ...
            if (dev & EDIT_MODE) {
                ...
                if (ev.mouse_button) {
                    if (selected_object) {
                        ...
                        oedit = wm->new_window(prop->getd("oedit x", 0), prop->getd("oedit y", 0), -1, -1,
                            new button_box(0, 0, ID_NULL, 1,
                                ...
                                new button(bw * 0, bh,         DEV_OEDIT_DELETE, cash.img(dev_del),
                                NULL)))))),
                            new button(bw *5, bh * 0, DEV_OEDIT_AI, cash.img(dev_ai),
                            
                            new button_box(bw * 1, bh * 1, DEV_OEDIT_CHAR_BOX, 0,
                                new button(0,  0, DEV_OEDIT_LEFT,     cash.img(dev_char_left),
                                new button(bw, 0, DEV_OEDIT_RIGHT,    cash.img(dev_char_right), NULL)),
                        ...
                        int bw=20+6,bh=16+6,wl=0, wh = 0,th=wm->font()->height()+4;
                        edit_light=selected_light;
                        if (edit_object) {
    ...
    case EV_MESSAGE : {
            ...
            case ID_CACHE_PROFILE : {
                    if (current_level && !cash.prof_is_on()) {
                        ...
                        the_game->show_help("Cache profiling already on.");
                } break;
            ...
            case ID_SET_SCROLL_CHECK : {
                    ...
                    if ((((float)tbg_xmul/(float)tbg_xdiv) < ((float)bg_xmul/(float)bg_xdiv)) ||
                        ...
                        int wl = 0, wt = 0, h = wm->font()->height()+8;
                        
                        warn_win = wm->new_window(xres/2-40,yres/2-40,-1,-1,
                            new info_field(wl, wt, ID_NULL, symbol_str("back_loss"),
...
pal_win::pal_win(void *args) 
{
    ...
    while (!NILP(ao)) {  // loop until we run out of parameters
        if (i>w*h) {
            lbreak("too many parameters to add_palette");
            //js exit(0);
        }
        pat[i] = (unsigned short) lnumber_value(CAR(ao));
        ...
        case EV_MOUSE_BUTTON :
            if (ev.mouse_button==1) {
                ...
                if (selx>=0 && sely>=0 && selx<w && sely<h) {
                    ...
                    if (dev_cont->forew)      
                        ((tile_picker *)dev_cont->forew->read(DEV_FG_PICKER))->recenter(dev_cont->forew->screen);
                }
            }
...
static pmenu *make_menu(int x, int y)
{
    return new pmenu(x,y,
        ...
        new pmenu_item(symbol_str("window_top"),new psub_menu(i_recurse(WinMenus),NULL),NULL))),screen);
}
