/*
 ...
 src/net/unix/undrv.c
  ...
  of abuse and therefore is a bit simpler.
*/
...
/* --------------------------------------------------------------------------- 
    Find a usable protocol from installed list 
 ------------------------------------------------------------------------- /**/
int ProtocolExists() {
    net_protocol *n = net_protocol::first;
    for (; n; n = n->next)
        if (n->installed())
            return 1;
    return 0;
    ...
    for (i=1;i<argc;i++) 
        if (!strcmp(argv[i],"-nonet"))
            ...
            if (x>=1 && x<=8)
                ...
                dprintf("bad value for min_players use 1..8\n");
        }
        ...
        if (!usable) {
            dprintf("No network protocols installed\n");
            return 0;
        }
        prot = usable;
        ...
        if (main_net_cfg->state == net_configuration::CLIENT) {
            ...
            if (!net_server) { 
                WriteErr(0, symbol_str("unable_locate"));
            }
...
int NF_set_file_server(net_address *addr)
{
    if (prot) {
        ...
        if (!sock) {
            dprintf("set_file_server::connect failed\n");
            return 0;
        }
        ...
        if (sock->write(&cmd, 1) == 1)
            if (sock->read(&cmd, 1) == 1) {
                ...
                dprintf("set_file_server::write failed\n");
            delete sock; 
            return 0;
    } 
    else
            ...
            if (comm_sock && comm_sock->ready_to_read()) { // new connection?
                ...
                if (new_sock) {
                    ...
                    if (new_sock->read(&client_type, 1) != 1) {
                        ...
                        uchar len;
                        _RPT1(_CRT_WARN, "Service_Net_Request: client_type = %d\n", client_type);
                        switch (client_type) {
                        case CLIENT_NFS:
                            delete addr;	
...
int request_server_entry()
{
    if (prot && main_net_cfg) {
        ...
        ushort port = main_net_cfg->port + 1, cnum;
        ...
        if (net_server) {
            ...
            while (join_list) {
                ...
                for (i = 0; i < total_objects; i++)
                    ...
                    f->next->SetTint(f->next->player_number);
            ...
            jwindow *j = wm->new_window(0, yres / 2, -1, -1,
                                        new info_field(WINDOW_FRAME_LEFT, WINDOW_FRAME_TOP, 0, symbol_str("resync"),
                                        new button(WINDOW_FRAME_LEFT, WINDOW_FRAME_TOP + wm->font()->height() + 5, ID_NET_DISCONNECT,
    ...
    if (prot && base->input_state != INPUT_PROCESSING) {     // if input is not here, wait on it
        ...
        while (base->input_state != INPUT_PROCESSING) {
            ...
            if (now.diff_time(&start) > 0.05) {
                ...
                if (total_retry == 100) {   // 2 minutes and nothing
                    if (main_net_cfg->state == net_configuration::SERVER)
                        abort = wm->new_window(0, yres / 2, -1, wm->font()->height() * 4,
                            new info_field(WINDOW_FRAME_LEFT, WINDOW_FRAME_TOP, 0, symbol_str("waiting"),
                            new button(WINDOW_FRAME_LEFT, WINDOW_FRAME_TOP + wm->font()->height() + 5, ID_NET_DISCONNECT, symbol_str("slack"), NULL)),
                            symbol_str("Error"));
                    else
                        abort = wm->new_window(0, yres / 2, -1, wm->font()->height() * 4,
                            new info_field(WINDOW_FRAME_LEFT, WINDOW_FRAME_TOP, 0, symbol_str("waiting"),
                            new button(WINDOW_FRAME_LEFT, WINDOW_FRAME_TOP + wm->font()->height() + 5, ID_NET_DISCONNECT, "Disconnect", NULL)),
                            symbol_str("Error"));
                }
            }
            if (total_retry == 1500) { //JS
                kill_slackers();
                base->input_state = INPUT_PROCESSING;
            } // /JS
            if (abort) {
