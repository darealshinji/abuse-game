...
public : 
    ...
    { 
        if (x < 0 || x >= nbacktiles || backtiles[x] < 0) 
            ...
            return cash.backt(backtiles[x]); 
    }
    foretile *get_fg(int x) 
    { 
        if (x < 0 || x >= nforetiles || foretiles[x] < 0) 
            return cash.foret(foretiles[BLACK]); 
        else 
    ...
    void show_time();
    tile_type get_map_bg(int x, int y) 
    { 
        return current_level->get_bg(x, y); 
    }
    tile_type get_map_fg(int x, int y) 
    { 
        return current_level->get_fg(x, y); 
    }
    void end_session();
    void need_refresh() // for development mode only
    { 
        refresh=1; 
    }       
