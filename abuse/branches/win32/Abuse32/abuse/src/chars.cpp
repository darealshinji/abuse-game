...
/* --------------------------------------------------------------------------- 
Adds a variable to a character. 
------------------------------------------------------------------------- /**/
...
character_type::character_type(void *args, void *name) 
{
    ...
    void *field;
    p_ref r7(field);
    for (field=args; field; field = CDR(field)) {
        ...
        if (f == l_abil) {
            ...
            p_ref r4(l);
            char fn[100];
            strcpy(fn, lstring_value(eval(CAR(l)))); 
            l = CDR(l);
            while (l) {
	            int index;
	            void *e;
