...
jwindow *create_num_window(int mx, int total_saved, image **thumb_nails)
{
    ...
    int iw = cash.img(save_buts[0])->width();
    int x = 0;
    ...
    return wm->new_window(mx, yres / 2 - (jwindow::TopBorder() + ih * 5) / 2, -1, -1, buts[0]);
}
...
int load_game(int show_all, char *title)   // return 0 if the player escapes, else return the number of the game to load
{
    ...
    for (start_num = 0; start_num < MAX_SAVE_GAMES; start_num++) {
        ...
        if (fp->open_failure())
            ...
            if (se && se->type == SPEC_IMAGE) {
                ...
                if (thumb_nails[start_num]->width() > max_w)
                    max_w = thumb_nails[start_num]->width();
                if (thumb_nails[start_num]->height() > max_h)
                    max_h = thumb_nails[start_num]->height();
                if (!first)
    ...
    jwindow *preview = wm->new_window(l_win->x + l_win->l + 5, l_win->y, max_w + 1, max_h, NULL, title);
