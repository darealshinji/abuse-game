...
static void *collect_array(void *x)
{
    ...
    lisp_1d_array *a=new_lisp_1d_array((unsigned short) s, NULL);
    void **src,**dst;
...
inline void *collect_cons_cell(void *x)
{
    ...
    for (;x && item_type(x)==L_CONS_CELL;)
    {
        ...
        p->car = (cons_cell*) collect_object(old_car); 
        p->cdr = (cons_cell*) collect_object(old_cdr); 
    ...
    if (x)
        last->cdr = (cons_cell*) collect_object(x);
...
static void *collect_object(void *x)
{
    ...
    if (((uchar *)x)>=cstart && ((uchar *)x)<cend) {
        ...
        case L_OBJECT_VAR :
            {
                ret=new_lisp_object_var( (short) ((lisp_object_var *)x)->number);
            } break;
    ...
    } else if ((uchar *)x<collected_start || (uchar *)x>=collected_end)  
    {
        if (item_type(x)==L_CONS_CELL) // still need to remap cons_cells outside of space
        {
            for (;x && item_type(x)==L_CONS_CELL;x=CDR(x))
                ((cons_cell *)x)->car = (cons_cell*) collect_object(((cons_cell *)x)->car);
            if (x)
                ((cons_cell *)x)->cdr = (cons_cell*) collect_object(((cons_cell *)x)->cdr);
        }
    }
...
{
    if (root) {
        root->value = (lisp_value*) collect_object(root->value);
        root->function = (lisp_function*) collect_object(root->function);
        root->name = (lisp_string*) collect_object(root->name);
