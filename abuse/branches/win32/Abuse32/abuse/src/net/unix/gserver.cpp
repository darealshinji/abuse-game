...
void game_server::add_engine_input()
{
    ...
    base->packet.set_tick_received((unsigned char) base->current_tick);
...
int game_server::process_client_command(player_client *c)
{
    ...
    case CLCMD_UNJOIN :
        {
            c->comm->write(&cmd,1);        // don't care whether this works or not
            c->set_delete_me(1);
...
int game_server::add_client(int type, net_socket *sock, net_address *from)
{
    if (type == CLIENT_ABUSE) {
        ...
        ushort our_port = main_net_cfg->port+1, cport;
        char name[256];
        uchar len;
        short nkills = main_net_cfg->kills;
