...
{
    switch (xx) {
        case 0 : set_fade_dir((signed char) v); break;
        case 1 : set_frame_dir((signed char) v); break;
        case 2 : direction = (signed char) v; break;
        case 3 : set_gravity(v); break;
        case 4 : set_fade_count((unsigned char) v); break;
        case 5 : set_fade_max((unsigned char) v); break;
        case 6 : active = (signed char) v; break;
        case 7 : set_flags((unsigned char) v); break;
        case 8 : set_aitype((unsigned char) v); break;
        case 9 : set_xvel(v); break;
        
        case 10 : set_fxvel((unsigned char) v); break;
        case 11 : set_yvel(v); break;
        case 12 : set_fyvel((unsigned char) v); break;
        case 13 : set_xacel(v); break;
        case 14 : set_fxacel((unsigned char) v); break;
        
        case 15 : set_yacel(v); break;
        case 16 : set_fyacel((unsigned char) v); break;
        case 17 : x=v; break;
        case 18 : set_fx((unsigned char) v); break;
        case 19 : y=v; break;
        
        case 20 : set_fy((unsigned char) v); break;
        case 21 : set_hp((unsigned char) v); break;
        case 22 : set_mp((unsigned char) v); break;
        case 23 : set_fmp((unsigned char) v); break;
        
        case 24 : current_frame = (unsigned char) v;  break;
        case 25 : set_aistate((unsigned char) v); break;
        
        case 26 : set_aistate_time((unsigned char) v); break;
        case 27 : set_targetable((unsigned char) v); break;
    }
}
...
int game_object::decide()
{
    if (figures[otype]->get_fun(OFUN_AI)) {
        ...
        if (profiling()) {
            time_marker now;
            profile_add_time(this->otype, (float) now.diff_time(prof1));
            delete prof1;
        }
...
int game_object::can_hurt(game_object *who)     // collision checking will ask first to see if you
{
    ...
    // I.E. don't let them hurt themselves, this can change if you override this virtual function
    
    if (who->hurtable() && ((Team == -1) || (Team != who->GetTeam())) && (is_attacker || current_level->is_attacker(who) || hurt_all()))
        return 1;
    else
        return 0;
}
...
/* --------------------------------------------------------------------------- 
 ------------------------------------------------------------------------- /**/
void game_object::do_damage(int amount, game_object *from, long hitx, long hity, long push_xvel, long push_yvel) 
{
    if ((Team != -1) && (Team == from->GetTeam()))
        return;
    ...
    if (d) {
        ...
        ((cons_cell *)am)->car = (cons_cell*) new_lisp_number(amount);   
        
        frm=new_cons_cell();
        l_ptr_stack.push(&frm);
        
        ((cons_cell *)frm)->car = (cons_cell*) new_lisp_pointer(from);
        
        hx=new_cons_cell();
        l_ptr_stack.push(&hx);
        
        ((cons_cell *)hx)->car = (cons_cell*) new_lisp_number(hitx);
        
        hy=new_cons_cell();
        l_ptr_stack.push(&hy);
        ((cons_cell *)hy)->car = (cons_cell*) new_lisp_number(hity);
        
        px=new_cons_cell();
        l_ptr_stack.push(&px);
        ((cons_cell *)px)->car = (cons_cell*) new_lisp_number(push_xvel);
        
        py=new_cons_cell();
        l_ptr_stack.push(&py);
        ((cons_cell *)py)->car = (cons_cell*) new_lisp_number(push_yvel);
        
        
        ((cons_cell*) am)->cdr  = (cons_cell*) frm;
        ((cons_cell*) frm)->cdr = (cons_cell*) hx;
        ((cons_cell*) hx)->cdr  = (cons_cell*) hy;
        ((cons_cell*) hy)->cdr  = (cons_cell*) px;
        ((cons_cell*) px)->cdr  = (cons_cell*) py;
...
/* --------------------------------------------------------------------------- 
 ------------------------------------------------------------------------- /**/
void game_object::damage_fun(int amount, game_object *from, long hitx, long hity, long push_xvel, long push_yvel)
{ 
    if (!hurtable() || !alive())
        return;
...
game_object *create(int type, long x, long y, int skip_constructor, int aitype)
{
    ...
    if (figures[type]->get_fun(OFUN_CONSTRUCTOR) && !skip_constructor) {
        ...
        if (profiling()) {
            time_marker now;
            profile_add_time(type, (float) now.diff_time(prof1));
            delete prof1;
        }
        restore_heap(m,TMP_SPACE);
        current_object=o;
...
{  
    ...
    if (figures[otype]->get_fun(OFUN_MOVER)) {     // is a lisp move function defined?
        ...
        ((cons_cell *)lcx)->car = (cons_cell*) new_lisp_number(cx);
        
        lcy=new_cons_cell();
        l_ptr_stack.push(&lcy);
        ((cons_cell *)lcy)->car = (cons_cell*) new_lisp_number(cy);
        
        lb=new_cons_cell();
        l_ptr_stack.push(&lb);
        ((cons_cell *)lb)->car = (cons_cell*) new_lisp_number(button);
        
        
        ((cons_cell *)lcx)->cdr = (cons_cell*) lcy;
        ((cons_cell *)lcy)->cdr = (cons_cell*) lb;
...
void game_object::change_type(int new_type)
{
    ...
    if (figures[new_type]->get_fun(OFUN_CONSTRUCTOR)) {
        ...
        if (profiling()) {
            time_marker now;
            profile_add_time(otype, (float) now.diff_time(prof1));
            delete prof1;
        }
