...
void write_wav(char *filename, long sample_rate, long data_size, unsigned char *data)
{
    ...
    if (fp->open_failure()) {
        sprintf(ErrMsg, "Unable to open %s for writing\n");
        delete fp;
        WriteErr(0, NULL);
    }
...
unsigned char *read_wav(char *filename, long &sample_rate, long &data_size)
{
    ...
    WriteErr(data,"Malloc error");
    
    sample_rate = pcm.wf.samplesps;     
//    fp->read(data, tag.size);
    WriteErr(fp->read(data, tag.size) == (signed) tag.size,"Premature end of file");
    WriteErr(pcm.bitsps == 8,"Only 8-bit samples supported");
    WriteErr(pcm.wf.channels == 1,"Only mono samples supported");  
    WriteErr(pcm.wf.align == 1,"Bad block allignment");
    delete fp;
    return data;
}