...
#define EV_SCREEN_REFRESH 2048

#define DI_ACQUIRE_STATE   1
#define DI_UNACQUIRE_STATE 2
extern int SetDIAcquire;
...
public:
    ...
    void set_mouse_shape(image *im, int centerx, int centery) {
        mouse->set_shape(im, -centerx, -centery);
    }
    void set_mouse_position(int mx, int my) {
        if (mouse)
            mouse->set_position(mx, my);
    }
    ~event_handler();
};
