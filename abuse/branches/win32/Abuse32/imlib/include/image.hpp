...
public:  
    ...
    __inline void get_clip(short &x1, short &y1, short &x2, short &y2) {
        x1=clipx1;
        y1=clipy1;
        x2=clipx2;
        y2=clipy2;
...
class image : public linked_node {
    ...
    void delete_page();
    void CreateDDSurface(int mode = 0);
...
public:
    ...
    image(short width, short height,                 // required
        ...
        short create_descriptor = 0);        // 0 = no, 1 = yes, 2 = yes & keep dirties
                                           // 8 = Keep DD Surface in system memory
    ...
    __inline unsigned char *Lock() {
        ...
        hRet = Surf->Lock(NULL, &LockDesc, 0, NULL);
        if (hRet == DD_OK) {
            locked = 1;
            return (unsigned char*) LockDesc.lpSurface;
        }
        else {
            locked = 0;
            return NULL;
    ...
    __inline unsigned char *scan_line(int y) {
        if (locked)
            return (unsigned char*) LockDesc.lpSurface + y * LockDesc.lPitch;
        else
            return data + y * w;
    }
    ...
    unsigned char *next_line(short lasty, unsigned char *last_scan) {
        if (!(Surf && locked))
            ...
            return last_scan + LockDesc.lPitch;
    }
    ...
    __inline short width() {
    /*        if (locked)
    return (short) LockDesc.lPitch;
        else */
        return (short) w;
    }
    __inline short height () {
        return (short) h;
    }
    __inline int Pitch() {
        if (locked)
            return LockDesc.lPitch;
        else
            return w;
    }
    ...
    __inline void  add_dirty          (int x1, int y1, int x2, int y2) {};
    //js { if (special) special->add_dirty(x1,y1,x2,y2); }
    __inline void  delete_dirty       (int x1, int y1, int x2, int y2) {};
    //js { if (special) special->delete_dirty(x1,y1,x2,y2); }
    __inline void  clear_dirties      () {}; //js { if (special) special->clear_dirties(); }
