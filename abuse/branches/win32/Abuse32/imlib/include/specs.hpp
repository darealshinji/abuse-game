...
protected:
    ...
    size_t flush_writes();                                       // returns 0 on failure, else # of bytes written
    ...
    virtual int unbuffered_seek(long offset, int whence)  = 0;   // whence = SEEK_SET, SEEK_CUR, SEEK_END, ret=0=success
...
public:
    ...
    // these read and writes, always read/write Intel endian-ness
...
public:
    ...
    unsigned char type;
    spec_entry(unsigned char spec_type, char *object_name, char *link_filename, unsigned long data_size, unsigned long data_offset) {
        type = spec_type;
