...
/* --------------------------------------------------------------------------- 
 ------------------------------------------------------------------------- /**/
__inline unsigned char *trans_image::clip_y(image *screen, int x1, int y1, int x2, int y2, int x, int &y, int &ysteps)
{
...
/* --------------------------------------------------------------------------- 
 ------------------------------------------------------------------------- /**/
void trans_image::put_image_filled(image *screen, int x, int y, uchar fill_color)
{
    short x1, y1, x2, y2;
    ...
    if (!datap)
        return;     // if clip_y says nothing to draw, return
  
    screen->Lock();
    screen_line = screen->scan_line(y) + x;  
    //int sw = screen->width() - w;
    int sw = screen->Pitch() - w;
    x1 -= x;
    x2 -= x;
    for (; ysteps > 0; ysteps--) {
    ...
    for (; ysteps; ysteps--) {
        for (ix = 0; ix < w;) {
            ...
            if (s_off < screen->scan_line(0))
      	        printf("trans_image::put_image_offseted -- bad write");
            ...
            if (ix<w) {
                ...
                if (ix + slam_length < x1 || ix > x2) { // see if this run is totally clipped
                    ...
                    if (ix < x1) {               // the left side needs to be chopped ?
                        ...
                        if (chop_length >= slam_length) { // see if we chopped it all off
                            ix+=slam_length;             // yes, advance everything to the end of run
                            datap+=slam_length;	      
                            slam_length=0;	       
                        }
                        else {
...
/* --------------------------------------------------------------------------- 
 ------------------------------------------------------------------------- /**/
void trans_image::put_fade(image *screen, int x, int y, int frame_on, int total_frames, color_filter *f, palette *pal) 
{
    short x1, y1, x2, y2;
...
/* --------------------------------------------------------------------------- 
 ------------------------------------------------------------------------- /**/
void trans_image::put_fade_tint(image *screen, int x, int y, int frame_on, int total_frames, uchar *tint, color_filter *f, palette *pal)
{
    short x1, y1, x2, y2;
    int ix, slam_length, chop_length, ysteps;
