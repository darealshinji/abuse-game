...
{
    if (!fread(&PCX_header.manufactururer,1,1,fp))
        return 0;
    if (!fread(&PCX_header.version,1,1,fp))
        return 0;
    if (!fread(&PCX_header.encoding,1,1,fp))
        return 0;
    if (!fread(&PCX_header.bits_per_pixel,1,1,fp))
        return 0;
    ...
    if (!fread(PCX_header.palette, 1, 48, fp))
        return 0;
    if (!fread(&PCX_header.reserved, 1, 1, fp))
        return 0;
    if (!fread(&PCX_header.color_planes, 1, 1, fp))
        return 0;
    PCX_header.bytes_per_line = read_short(fp); 
    PCX_header.palette_type = read_short(fp); 
    if (!fread(PCX_header.filter, 1, 58, fp))
        return 0;
    return 1;
}
...
{
    if (PCX_file_type(filename) != PCX_24)
        return NULL;
    FILE *fp = fopen(filename, "rb");
    read_PCX_header(fp);
