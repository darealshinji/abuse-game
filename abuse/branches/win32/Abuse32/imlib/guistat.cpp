...
public:
    ...
    gui_status_node(char *Name, visual_object *Show, gui_status_node *Next) {
...
void gui_status_manager::draw_bar(gui_status_node *whom, int perc)
{
    ...
    whom->stat_win->screen->bar((short) (whom->stat_win->x1() + 2), (short) (whom->stat_win->y2() - h), (short) (whom->stat_win->x2() - 2),
		                        (short) (whom->stat_win->y2() - 2), (unsigned char) wm->dark_color());
    if (perc)
    ...
    if (first) {
        if (!first->stat_win) {
            ...
            if (now.diff_time(&first->last_time)>1) {
                long wx = xres / 2,
                     wy = 10,
                     len1 = strlen(first->name)*wm->font()->width() + 10,
                     len2 = 0,
                     len3,
                     h1 = wm->font()->height() + 5,
                     h2 = first->show ? first->show->height() : 0;
                if (first->show) 
